...
template<typename T>
constexpr Vec2<T>::Vec2(T _x, T _y)
  : x{_x}
  , y{_y}
{
}
...
template<typename T>
constexpr Vec2<T>::Vec2(const T (&_values)[2])
  : x{_values[0]}
  , y{_values[1]}
{
}
...
template<typename T>
T& Vec2<T>::operator[](Size _i) {
  RX_ASSERT(_i < 2, "out of bounds");
  return array[_i];
}

template<typename T>
const T& Vec2<T>::operator[](Size _i) const {
  RX_ASSERT(_i < 2, "out of bounds");
  return array[_i];
}

template<typename T>
bool Vec2<T>::is_any(T _value) const {
  return x == _value || y == _value;
}

template<typename T>
bool Vec2<T>::is_all(T _value) const {
  return x == _value && y == _value;
}
...
template<typename T>
T Vec2<T>::max_element() const {
  return Algorithm::max(x, y);
}

template<typename T>
T Vec2<T>::min_element() const {
  return Algorithm::min(x, y);
}

template<typename T>
template<typename F>
Vec2<T> Vec2<T>::map(F&& _fn) const {
  return { _fn(x), _fn(y) };
}
...
template<typename T>
template<typename T2>
constexpr Vec2<T2> Vec2<T>::cast() const {
...
template<typename T>
void Vec2<T>::operator*=(T _scalar) {
  x *= _scalar;
  y *= _scalar;
}
...
template<typename T>
constexpr bool operator==(const Vec2<T>& _lhs, const Vec2<T>& _rhs) {
...
template<typename T>
constexpr Vec2<T> operator-(const Vec2<T>& _v) {
  return {-_v.x, -_v.y};
}

template<typename T>
constexpr Vec2<T> operator+(const Vec2<T>& _a, const Vec2<T>& _b) {
...
template<typename T>
constexpr Vec2<T> operator*(T _scalar, const Vec2<T>& _v) {
  return {_scalar * _v.x, _scalar * _v.y};
}

template<typename T>
constexpr Vec2<T> operator*(const Vec2<T>& _v, T _scalar) {
  return _scalar * _v;
}
...
template<typename T>
constexpr bool operator<(const Vec2<T>& _a, const Vec2<T>& _b) {
  return _a.x < _b.x && _a.y < _b.y;
}
...
template<typename T>
constexpr T dot(const Vec2<T>& _lhs, const Vec2<T>& _rhs) {
  return _lhs.x * _rhs.x + _lhs.y * _rhs.y;
}
...
template<typename T>
Float32 distance(const Vec2f& _a, const Vec2f& _b) {
  return length(_a - _b);
}
