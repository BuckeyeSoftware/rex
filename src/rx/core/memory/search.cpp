...
void* search(const void* _haystack, Size _haystack_size, const void* _needle, Size _needle_size) {
  auto haystack = reinterpret_cast<const Byte*>(_haystack);
  auto needle = reinterpret_cast<const Byte*>(_needle);
  ...
  // For short needles, use fast search.
  haystack = static_cast<const Byte*>(memchr(_haystack, *needle, _haystack_size));
  if (!haystack || _needle_size == 1) {
    return (void*)haystack;
  }

  _haystack_size -= haystack - reinterpret_cast<const Byte*>(_haystack);
  if (_haystack_size < _needle_size) {
    return nullptr;
  }
