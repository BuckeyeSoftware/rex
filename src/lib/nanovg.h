...
#ifdef NANOVG_IMPLEMENT_FONTSTASH
#define FONTSTASH_IMPLEMENTATION
#endif
#include "fontstash.h"

#ifdef NANOVG_IMPLEMENT_STBIMAGE
...
#ifndef nvg__memset
#include <string.h>
#define nvg__memset memset
#endif

#ifndef nvg__memcpy
#include <string.h>
#define nvg__memcpy memcpy
#endif

#ifndef nvg__malloc
#include <stdlib.h>
#define nvg__malloc malloc
#define nvg__realloc realloc
#define nvg__free free
#endif
...
// static float nvg__sqrtf(float a) { return Rx::Math::sqrt(a); }
// static float nvg__modf(float a, float b) { return Rx::Math::mod(a, b); }
// static float nvg__sinf(float a) { return Rx::Math::sin(a); }
// static float nvg__cosf(float a) { return Rx::Math::cos(a); }
// static float nvg__tanf(float a) { return Rx::Math::tan(a); }
// static float nvg__atan2f(float a,float b) { return Rx::Math::atan2(a, b); }
// static float nvg__acosf(float a) { return Rx::Math::acos(a); }
// static float nvg__ceilf(float a) { return Rx::Math::ceil(a); }
...
{
	if (c == NULL) return;
	if (c->points != NULL) nvg__free(c->points);
	if (c->paths != NULL) nvg__free(c->paths);
	if (c->verts != NULL) nvg__free(c->verts);
	nvg__free(c);
}

static NVGpathCache* nvg__allocPathCache(void)
{
	NVGpathCache* c = (NVGpathCache*)nvg__malloc(sizeof(NVGpathCache));
	if (c == NULL) goto error;
	nvg__memset(c, 0, sizeof(NVGpathCache));

	c->points = (NVGpoint*)nvg__malloc(sizeof(NVGpoint)*NVG_INIT_POINTS_SIZE);
	if (!c->points) goto error;
	c->npoints = 0;
	c->cpoints = NVG_INIT_POINTS_SIZE;

	c->paths = (NVGpath*)nvg__malloc(sizeof(NVGpath)*NVG_INIT_PATHS_SIZE);
	if (!c->paths) goto error;
	c->npaths = 0;
	c->cpaths = NVG_INIT_PATHS_SIZE;

	c->verts = (NVGvertex*)nvg__malloc(sizeof(NVGvertex)*NVG_INIT_VERTS_SIZE);
...
NVGcontext* nvgCreateInternal(NVGparams* params)
{
	...
	NVGcontext* ctx = (NVGcontext*)nvg__malloc(sizeof(NVGcontext));
	int i;
	if (ctx == NULL) goto error;
	nvg__memset(ctx, 0, sizeof(NVGcontext));

	ctx->params = *params;
	for (i = 0; i < NVG_MAX_FONTIMAGES; i++)
		ctx->fontImages[i] = 0;

	ctx->commands = (float*)nvg__malloc(sizeof(float)*NVG_INIT_COMMANDS_SIZE);
	...
	// Init font rendering
	nvg__memset(&fontParams, 0, sizeof(fontParams));
...
void nvgDeleteInternal(NVGcontext* ctx)
{
	int i;
	if (ctx == NULL) return;
	if (ctx->commands != NULL) nvg__free(ctx->commands);
...
{
	float s2[6];
	nvg__memcpy(s2, s, sizeof(float)*6);
	nvgTransformMultiply(s2, t);
	nvg__memcpy(t, s2, sizeof(float)*6);
}
...
void nvgSave(NVGcontext* ctx)
{
	if (ctx->nstates >= NVG_MAX_STATES)
		return;
	if (ctx->nstates > 0)
		nvg__memcpy(&ctx->states[ctx->nstates], &ctx->states[ctx->nstates-1], sizeof(NVGstate));
	ctx->nstates++;
}
...
void nvgReset(NVGcontext* ctx)
{
	...
	nvg__memset(state, 0, sizeof(*state));
...
void nvgCurrentTransform(NVGcontext* ctx, float* xform)
{
	...
	nvg__memcpy(xform, state->xform, sizeof(float)*6);
}
...
void nvgIntersectScissor(NVGcontext* ctx, float x, float y, float w, float h)
{
	...
	nvg__memcpy(pxform, state->scissor.xform, sizeof(float)*6);
...
{
	NVGstate* state = nvg__getState(ctx);
	nvg__memset(state->scissor.xform, 0, sizeof(state->scissor.xform));
	state->scissor.extent[0] = -1.0f;
	state->scissor.extent[1] = -1.0f;
}
...
static void nvg__appendCommands(NVGcontext* ctx, float* vals, int nvals)
{
	...
	if (ctx->ncommands+nvals > ctx->ccommands) {
		...
		commands = (float*)nvg__realloc(ctx->commands, sizeof(float)*ccommands);
	...
	nvg__memcpy(&ctx->commands[ctx->ncommands], vals, nvals*sizeof(float));
...
static void nvg__addPath(NVGcontext* ctx)
{
	...
	if (ctx->cache->npaths+1 > ctx->cache->cpaths) {
		...
		paths = (NVGpath*)nvg__realloc(ctx->cache->paths, sizeof(NVGpath)*cpaths);
		if (paths == NULL) return;
		ctx->cache->paths = paths;
		ctx->cache->cpaths = cpaths;
	}
	path = &ctx->cache->paths[ctx->cache->npaths];
	nvg__memset(path, 0, sizeof(*path));
...
static void nvg__addPoint(NVGcontext* ctx, float x, float y, int flags)
{
	...
	if (ctx->cache->npoints+1 > ctx->cache->cpoints) {
		...
		points = (NVGpoint*)nvg__realloc(ctx->cache->points, sizeof(NVGpoint)*cpoints);
...
static NVGvertex* nvg__allocTempVerts(NVGcontext* ctx, int nverts)
{
	if (nverts > ctx->cache->cverts) {
		...
		verts = (NVGvertex*)nvg__realloc(ctx->cache->verts, sizeof(NVGvertex)*cverts);
...
static int nvg__curveDivs(float r, float arc, float tol)
{
	...
	return nvg__maxi(2, (int)nvg__ceilf(arc / da));
}
								 ...
								 float fringe)
{
	...
	if (p1->flags & NVG_PT_LEFT) {
		...
		n = nvg__clampi((int)nvg__ceilf(((a0 - a1) / NVG_PI) * ncap), 2, ncap);
		for (i = 0; i < n; i++) {
			float u = i/(float)(n-1);
			float a = a0 + u*(a1-a0);
		...
		n = nvg__clampi((int)nvg__ceilf(((a1 - a0) / NVG_PI) * ncap), 2, ncap);
		for (i = 0; i < n; i++) {
			float u = i/(float)(n-1);
			float a = a0 + u*(a1-a0);
